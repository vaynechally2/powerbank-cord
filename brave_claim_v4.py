...
def hwnd_exists(hwnd: int) -> bool:
    try:
        return any(getattr(w, "_hWnd", None) == hwnd for w in gw.getAllWindows())
    except Exception:
        return False
...
class Bot:
    ...
    def log_txt(self, cfg, msg):
        ...
        with open(cfg["log_txt_path"], "a", encoding="utf-8") as f:
            f.write(line)
    ...
    def log_csv(self, cfg, event, detail=""):
        ...
        with open(cfg["log_csv_path"], "a", newline="", encoding="utf-8") as f:
            ...
            w.writerow([ts(cfg["clock_offset_sec"]), event, detail])
    ...
    def try_selector_or_text_click(self, page, cfg):
        ...
        for sel in split_lines(cfg["selectors_multiline"]):
            try:
        ...
        for txt in split_lines(cfg["fallback_texts_multiline"]):
            try:
    ...
    def try_image_click_fallback(self, cfg):
        ...
        w = get_window_by_hwnd(self.bound_hwnd) if self.bound_hwnd else None
        if not w:
    ...
    def run(self, cfg):
        ...
        while not self.stop_event.is_set():
            try:
                    ...
                    while not self.stop_event.is_set():
                        ...
                        if now >= next_refresh:
                            try:
                                ...
                                self.log_txt(cfg, f"REFRESH_ERR: {e}")
                                self.log_csv(cfg, "REFRESH_ERR", str(e))
                ...
                self.log_txt(cfg, f"CRASH_RECOVERY #{crashes}: {e} (retry in {wait_s}s)")
                self.log_csv(cfg, "CRASH_RECOVERY", f"{e}|wait={wait_s}")
...
class App:
    ...
    def cfg(self):
        ...
        if not os.path.exists(c["brave_exe_path"]):
            raise ValueError("Brave EXE path not found.")
        if not os.path.exists(c["brave_user_data_dir"]):
    ...
    def on_test(self):
        try:
            c = self.cfg()
            threading.Thread(target=self.bot.test_selector_once, args=(c,), daemon=True).start()
        except Exception as e:
            messagebox.showerror(APP_NAME, str(e))
    ...
    def list_profiles(self):
        ...
        with open(PROFILES_FILE, "r", encoding="utf-8") as f:
            data = json.load(f)
        for k in data.keys():
            self.profile_list.insert("end", f"- {k}\n")
    ...
    def build(self):
        ...
        t3 = ttk.Frame(nb)
        nb.add(t1, text="Main")
        nb.add(t2, text="Claim Detection")
        nb.add(t3, text="Profiles")

        self.add_entry(t1, "Brave EXE path", "brave_exe_path", r"C:\Program Files\BraveSoftware\Brave-Browser\Application\brave.exe")
        self.add_entry(t1, "Brave user data dir", "brave_user_data_dir", os.path.expandvars(r"%LOCALAPPDATA%\BraveSoftware\Brave-Browser\User Data"))
        self.add_entry(t1, "Brave profile directory", "brave_profile_dir", "Default")
        self.add_entry(t1, "Target URL", "target_url", "https://example.com/path")
        self.add_entry(t1, "Target URL prefix", "target_url_prefix", "https://example.com/")
        self.add_entry(t1, "Strict URL match (true/false)", "strict_url_match", "false")
        self.add_entry(t1, "Domain whitelist CSV", "domain_whitelist_csv", "example.com")
        self.add_entry(t1, "Refresh interval sec", "refresh_interval_sec", "10")
        self.add_entry(t1, "Refresh jitter +/- sec", "refresh_jitter_sec", "0.5")
        self.add_entry(t1, "Refresh min sec", "refresh_min_sec", "2")
        self.add_entry(t1, "Refresh max sec", "refresh_max_sec", "300")
        self.add_entry(t1, "Loop sleep sec", "loop_sleep_sec", "0.25")
        self.add_entry(t1, "Click cooldown sec", "click_cooldown_sec", "25")
        self.add_entry(t1, "Allowed start HH:MM", "allowed_start", "08:00")
        self.add_entry(t1, "Allowed end HH:MM", "allowed_end", "23:00")
        self.add_entry(t1, "Use weekday/weekend schedule", "use_weekday_weekend_schedule", "true")
        self.add_entry(t1, "Weekday start HH:MM", "weekday_start", "08:00")
        self.add_entry(t1, "Weekday end HH:MM", "weekday_end", "23:00")
        self.add_entry(t1, "Weekend start HH:MM", "weekend_start", "09:00")
        self.add_entry(t1, "Weekend end HH:MM", "weekend_end", "22:00")
        self.add_entry(t1, "Clock offset sec (+ahead -behind)", "clock_offset_sec", "0.0")
        self.add_entry(t1, "Pause on user activity", "pause_on_user_activity", "true")
        self.add_entry(t1, "Idle required sec", "user_idle_required_sec", "3")
        self.add_entry(t1, "Screenshot on click", "screenshot_on_click", "true")
        self.add_entry(t1, "Screenshot directory", "screenshot_dir", "screenshots")
        self.add_entry(t1, "TXT log path", "log_txt_path", "bot.log")
        self.add_entry(t1, "CSV log path", "log_csv_path", "bot.csv")
        self.add_entry(t1, "Max crash retries", "max_crash_retries", "8")
        ...
        ttk.Label(t1, textvariable=self.status_var, foreground="blue").pack(anchor="w", padx=10, pady=2)

        self.add_text(t2, "CSS selectors (Inspect -> Copy selector, one per line)", "selectors_multiline", "button[data-test='claim']\nbutton:has-text('Claim')")
        self.add_text(t2, "Fallback texts (one per line)", "fallback_texts_multiline", "Claim\nCollect\nGet reward")
        self.add_entry(t2, "Use image fallback", "use_image_fallback", "false")
        self.add_entry(t2, "Reference image path (optional)", "reference_image_path", "")
        self.add_entry(t2, "Image confidence 0.1-1.0", "image_confidence", "0.82")

        r2 = ttk.Frame(t3)
        r2.pack(fill="x", padx=6, pady=6)
        ttk.Label(r2, text="Profile name").pack(side="left")
        ttk.Entry(r2, textvariable=self.profile_var, width=24).pack(side="left", padx=6)
        ttk.Button(r2, text="Save", command=self.save_profile).pack(side="left", padx=3)
        ttk.Button(r2, text="Load", command=self.load_profile).pack(side="left", padx=3)
        ttk.Button(r2, text="List", command=self.list_profiles).pack(side="left", padx=3)

        self.profile_list = tk.Text(t3, height=20)
